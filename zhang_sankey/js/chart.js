async function drawChart() {
    ...
    const severityAccessor = d => d.severity
    ...
    const severityIds = d3.range(severityNames.length)

    const roadConditionAccessor = d => d.roadCondition
    const roadConditionNames = ["Excellent","Pretty good", "Good","Normal", "Bad","Very bad","Extremely bad"]
    const roadConditionIds = d3.range(roadConditionNames.length)

    const getStatusKey = ({sex, roadCondition}) => [sex, roadCondition].join("--")

    const stackedProbabilities = {}
    dataset.forEach(startingPoint => {
        const key = getStatusKey(startingPoint)
        let stackedProbability = 0
        stackedProbabilities[key] = severityNames.map((severity, i) => {
            stackedProbability += (startingPoint[severity] / 100)
    ...
    function generatePerson(elapsed) {
        ...
        const roadCondition = getRandomValue(roadConditionIds)
        const statusKey = getStatusKey({
            sex: sexes[sex],
            roadCondition: roadConditionNames[roadCondition],
        })
        const probabilities = stackedProbabilities[statusKey]
        const severity = d3.bisect(probabilities, Math.random())
    ...
    const startYScale = d3.scaleLinear()
        .domain([roadConditionIds.length, -1])
        .range([0, dimensions.boundedHeight])

    const endYScale = d3.scaleLinear()
        .domain([severityIds.length, -1])
        .range([0, dimensions.boundedHeight])
    ...
    const colorScale = d3.scaleLinear()
        .domain(d3.extent(roadConditionIds))
        .range(["#12CBC4", "#B53471"])
    ...
    const linkOptions = d3.merge(
        roadConditionIds.map(startId => (
            severityIds.map(endId => (
    ...
    const startingLabels = startingLabelsGroup.selectAll(".start-label")
        .data(roadConditionIds)
        .enter().append("text")
        .attr("class", "label start-label")
        .attr("y", (d, i) => startYScale(i))
        .text((d, i) => sentenceCase(roadConditionNames[i]))

    const startLabel = startingLabelsGroup.append("text")
        .attr("class", "start-title")
        .attr("y", startYScale(roadConditionIds[roadConditionIds.length - 1]) - 65)
        .text("Road ")
    const startLabelLineTwo = startingLabelsGroup.append("text")
        .attr("class", "start-title")
        .attr("y", startYScale(roadConditionIds[roadConditionIds.length - 1]) - 50)
        .text("Condition")
  ...
  const endLabel = startingLabelsGroup.append("text")
        .attr("class", "start-title")
      .attr("x", 965)
        .attr("y", startYScale(roadConditionIds[roadConditionIds.length - 1]) - 15)
        .text("Accident")
    const endLabelLineTwo = startingLabelsGroup.append("text")
        .attr("class", "start-title")
        .attr("x", 965)
        .attr("y", startYScale(roadConditionIds[roadConditionIds.length - 1]) )
        .text("Severity")
    ...
    function updateMarkers(elapsed) {
        ...
        markers.style("transform", d => {
            ...
            const yStart = startYScale(roadConditionAccessor(d))
            const yEnd = endYScale(severityAccessor(d))
            ...
            .attr("fill", d => colorScale(roadConditionAccessor(d)))
        ...
        const endingGroups = severityIds.map(endId => (
            people.filter(d => (
                xProgressAccessor(d) >= 1
                && severityAccessor(d) == endId
            ))
        ))
        const endingPercentages = d3.merge(
            endingGroups.map((peopleWithSameEnding, endingId) => (
                d3.merge(
                    sexIds.map(sexId => (
                        roadConditionIds.map(roadConditionId => {
                            ...
                            const peopleInBarWithSameStart = peopleInBar.filter(d => (
                                roadConditionAccessor(d) == roadConditionId
                            ))
